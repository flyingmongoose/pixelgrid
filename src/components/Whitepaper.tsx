...
const renderNestedList = (list: Element): React.ReactNode => {
  const ListComponent = list.tagName === 'ol' ? 'ol' : 'ul';
  const listClassName = list.tagName === 'ol' ? "list-decimal" : "list-disc";
  return (
    <ListComponent className={`${listClassName} list-inside ml-4 mt-2 space-y-2`}>
  ...
  const components: Partial<Components> = {
    ...
    li: ({node, children, ...props}) => {
      ...
      const hasNestedList = node.children.some(
        child => child.type === 'element' && (child.tagName === 'ul' || child.tagName === 'ol')
      );
